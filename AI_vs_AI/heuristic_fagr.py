...
def evaluation_function(board,turn,stealing=True):
    # Initial variables
    state = board
    player_turn = turn
    opposite_player_turn = 1 if player_turn == 0 else 0
    ...
    # Compute player 1 score
    player_1_score = sum(state[0:6])

    # Compute player 2 score
    player_2_score = sum(state[7:13])
...
def check_player_utility(player_turn, state):
    utility = 0
    opposite_player_turn = 1 if player_turn == 0 else 0
    start = 7 if player_turn == 1 else 0

    # Check steal and extra turn
    for slot, slot_count in enumerate(state[start:start+6]):
        # Can end in own slot
        ...
        pieces_to_steal = state[opp_start:opp_start+6][abs(5 - slot)]
        if slot_count == 0 and pieces_to_steal > 0:
            # Check if we can land in empty slot
            for slot2, slot_count2 in enumerate(state[start:start+6][0:slot]):
